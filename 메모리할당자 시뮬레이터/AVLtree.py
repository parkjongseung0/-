...
class AVLTree:
    ...
    def insert(self,x):
        self.__root = self.__insertItem(self.root,x)
    ...
    def deleteNode(self, tNode:AVLNode) -> AVLNode:
        if tNode.left == self.NIL and tNode.right == self.NIL: # case 1(자식이 없음)
            ...
            (rtnItem, rtnNode) = self ._
            tNode.item = rtnItem
            tNode.right = rtnNode
            tNode.height = 1 + max(tNode.right.height, tNode.left.height)
            type = self .__needBalance (tNode)
            if type != self.NO_NEED:
                tNode = self. balanceAVL(tNode, type)
            return tNode # tNode survived
        
    def deleteMinItem(self, tNode:AVLNode) -> tuple:
    ...
    def balanceAVL(self, tNode:AVLNode, type:int) -> AVLNode:
        returnNode = self.NIL
        if type == self.LL:
    ...
    def leftRotate(self, t:AVLNode) -> AVLNode:
        RChild = t.right
        if RChild == self.NIL:
    ...
    def _rightRotate(self, t:AVLNode) -> AVLNode:
        LChild = t.left
        if LChild == self.NIL:
    ...
    def needBalance(self, t:AVLNode) -> int:
        type = self. ILLEGAL
        if (t.left.height + 2 <= t.right.height): 
            if (t.right.left.height) <= t.right.right.height:
                ...
                type = self.RL
        elif (t.left.height) >= t.right.height + 2: # L 9%
