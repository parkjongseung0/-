...
class Allocator:
    ...
    def free(self, id):
        if id in self.used_chunks:
            ...
            # Check if the entire chunk can be freed back to the OS
            chunk_start = start // self.chunk_size
            chunk_end = (start + size) // self.chunk_size

            if chunk_start == chunk_end:
                # Check if this chunk is completely free
                free_in_chunk = [
                    (addr, sz) for addr, sz in self.used_chunks.values()
                    if addr // self.chunk_size == chunk_start
                ]

                if not free_in_chunk:
                    self.arena.pop()
