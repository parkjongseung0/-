...
class Allocator:
    def __init__(self):
        ...
        self.threshold = 64 * 1024  # Threshold for returning chunks to OS
    ...
    def free(self, id):
        if id in self.used_chunks:
            ...
            if self.total_allocated < len(self.arena) * self.chunk_size - self.threshold:
                self._return_chunks_to_os()
    ...
    def _return_chunks_to_os(self):
        free_memory = self.total_allocated
        required_chunks = (free_memory + self.chunk_size - 1) // self.chunk_size
        self.arena = self.arena[:required_chunks]
